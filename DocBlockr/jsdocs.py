...
class JsdocsParser:
    ...
    def formatVar(self, name, val):
        ...
        if self.inline:
            ...
            out.append("${1:@name        %s}" % (escape(name)))
            out.append("@fieldOf ${1:[Containing object]}")
            ...
            out.append("@default ${1:[Default value]}\n * ")
            out.append("@description ${1:[Description]}")
    ...
    def formatFunction(self, name, args, retval, options={}):
        ...
        description = self.getNameOverride() or ('@name        %s' % escape(name))
